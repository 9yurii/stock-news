...
NOISE_EXACT = {
    ...
    "선물", "삭제된 메시지입니다", "메시지가 삭제되었습니다.", "메시지가 삭제되었습니다",
    "지도", "연락처", "카카오톡 프로필",
    "<사진>", "<동영상>", "[사진]", "[동영상]",
...
# "사진 3장", "<사진 읽지 않음>", "파일: 보고서.pdf" 처럼 변형이 있는 자리표시
NOISE_PATTERNS = (
    re.compile(r"^사진\s*\d+장$"),
    re.compile(r"^<(사진|동영상|이모티콘|음성메시지|파일)(\s+읽지\s*않음)?>$"),
    re.compile(r"^파일\s*:\s*.+$"),
    re.compile(r"^샵검색\s*:\s*.+$"),
)

# 사진 자리표시로 세는 표기 (원본이 없으므로 몇 장인지 알려주기용)
PHOTO_COUNT_PATTERNS = (
    re.compile(r"^<사진(\s+읽지\s*않음)?>$"),
    re.compile(r"^<동영상(\s+읽지\s*않음)?>$"),
    re.compile(r"^(사진|동영상)$"),
    re.compile(r"^\[?(사진|동영상)\]?$"),
    re.compile(r"^(사진|동영상)을 보냈습니다\.?$"),
)
